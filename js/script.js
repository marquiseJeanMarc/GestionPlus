...
if (document.querySelector("input[name='lastName']") != null) 
{
	...
	let messPass1 = "Le mot de passe doit avoir : <ul id='ulMessageErreurDuMotDePasse'><li>Au moins une majuscule</li><li>Au moins un chiffre</li><li>Au moins 6 caractères</li><li>Au moins un caractère spécial</li></ul>";
...
if (document.querySelector("#cat-select") != null) {
	let categorie = document.querySelector("#cat-select");
	categorie.addEventListener('change',pickCat);
	
} 

function pickCat() {
    
	$catName = this.options[this.selectedIndex].text;
	/*let vueCategorie = document.querySelector("#vueCategorie");*/
	document.querySelector("#vueCategorie").innerText = $catName;
	/*console.log($catName);*/
    document.querySelector("input[name='catId']").value = this.value;
